...
export const FeedbackForm: FC<FeedbackFormProps> = ({ showResultPopUp }) => {
    ...
    useEffect(() => {
        ...
        if (messageTopicFromLocalStorage) {
            const messageTopicFromLocalStorageCopy = JSON.parse(
                messageTopicFromLocalStorage
            ).messageTopic;
    ...
    return (
        <FormProvider {...methods}>
            <form onSubmit={(e) => e.preventDefault()} noValidate className={s.feedbackForm}>
                <h2 className={s.feedbackFormHeading}>Связаться с нами</h2>
                <p className={s.feedbackFormDescription}>
                ...
                <div className={s.feedbackFields}>
                    ...
                    <div className={`${s.feedbackFormField} ${s.paddingTop}`}>
                        ...
                        <div ref={dropdownRef}>
                            ...
                            >
                                ...
                                <img
                                    src={openDropdown ? selectUpArrowIcon : selectDownArrowIcon}
                                    alt="down"
                                />
                            </button>
                            {openDropdown && (
                                <ul className={s.selectDropdown}>
                                    <li className={s.selectItem} onClick={(e) => handleSelect(e)}>
                                        Вопрос
                                    </li>
                                    <li className={s.selectItem} onClick={(e) => handleSelect(e)}>
                                        Предложение
                    ...
                    <div className={s.messageCounter}>
                        {methods.watch('messageContent')
                            ? methods.watch('messageContent').length
                            : '0'}
                        /500
                    </div>
                </div>
                <div className={s.feedbackFormPolytics}>
                    <button
                        type="button"
                        className={`${s.feedbackFormCheckbox}${agreed ? ' ' + s.selected : ''}`}
                    ...
                    <span>
                        ...
                        <span>
                            c <Link to={'/privacy-policy/'}>политикой конфиденциальности</Link>
                        </span>
                    </span>
                </div>
                ...
                <button
                    type="submit"
                    className={s.feedbackFormSubmitBtn}
                    disabled={!agreed}
                    onClick={onSubmit}
                >
                    Отправить
